...
function renderCurrentColor(): void {
  const values = [pickedColor.r, pickedColor.g, pickedColor.b].map((value) =>
    Math.max(0, Math.min(255, Math.round(value * 255))),
  );
  redValue.value = String(values[0]);
  greenValue.value = String(values[1]);
  blueValue.value = String(values[2]);
  redSlider.value = String(values[0]);
  greenSlider.value = String(values[1]);
  blueSlider.value = String(values[2]);
  redSliderValue.value = String(values[0]);
  greenSliderValue.value = String(values[1]);
  blueSliderValue.value = String(values[2]);
...
function rebuildVoxelMesh(): void {
  ...
  const positions = activeAnimation
    ? sampledAnimationPositions(activeAnimation, animationElapsed)
    : undefined;
...
function rebuildSkeleton(override?: Vec3[]): void {
  ...
  model.sticks.forEach((stick) => {
    const a = byId.get(stick.a);
    const b = byId.get(stick.b);
  ...
  pointGeometry.setAttribute(
    ...
    new THREE.Float32BufferAttribute(
      positions.flatMap((point) => [point.x, point.y, point.z]),
      3,
    ),
  );
  ...
  if (animationPage === 'edit' && positions[selectedParticleIndex]) {
    ...
    selectedGeometry.setAttribute(
      'position',
      new THREE.Float32BufferAttribute([selected.x, selected.y, selected.z], 3),
    );
...
function updateSelectionHelper(): void {
  if (selectionHelper) {
    ...
    const materials = Array.isArray(selectionHelper.material)
  ...
  box.min.addScalar(-0.55);
  box.max.addScalar(0.55);
...
function updateStats(): void {
  ...
  if (selected.length === 1) {
    ...
    const xs = selected.map((v) => v.x);
    const ys = selected.map((v) => v.y);
...
function cloneAnimation(animation: RwrAnimation, name: string): RwrAnimation {
  return {
    ...
    frames: animation.frames.map((frame) => ({
      time: frame.time,
      positions: clonePositions(frame.positions),
    })),
  };
}
...
function updateAnimationEditor(): void {
  ...
  [
    ...
    animationEndInput,
    animationSpeedInput,
    animationSpreadInput,
    animationLoopInput,
  ].forEach((input) => {
    input.disabled = !hasAnimation;
  });
  if (animation) {
    ...
    animationEndInput.value = '';
    animationSpeedInput.value = '';
    animationSpreadInput.value = '';
    animationLoopInput.checked = false;
  }
  ...
  frames.forEach((frame, index) => {
    const option = document.createElement('option');
    option.value = String(index);
    option.textContent = `第 ${index + 1} 帧 · ${frame.time.toFixed(3)}s`;
    keyframeSelect.appendChild(option);
  });
  if (!frames.length) {
    const option = document.createElement('option');
    option.textContent = '没有关键帧';
    keyframeSelect.appendChild(option);
  }
  ...
  model?.skeleton.forEach((particle, index) => {
    const option = document.createElement('option');
    option.value = String(index);
    option.textContent = `${index + 1}. ${particle.name || `骨骼点 ${particle.id}`}`;
    particleSelect.appendChild(option);
  });
  if (!model?.skeleton.length) {
    const option = document.createElement('option');
    option.textContent = '请先载入带骨骼的模型';
    particleSelect.appendChild(option);
  }
...
function updateParticleFields(): void {
  ...
  [particleXInput, particleYInput, particleZInput].forEach((input) => {
  ...
  autosaveTimer = window.setTimeout(() => {
    ...
    try {
      localStorage.setItem(
        'rwr-editor-recovery',
        JSON.stringify({ name: currentFileName, savedAt: Date.now(), xml: model.serialize() }),
      );
    } catch {
      /* 浏览器配额不足时不影响编辑 */
    }
...
function beginBoneDrag(event: PointerEvent): boolean {
  ...
  boneDragPlane.setFromNormalAndCoplanarPoint(
    boneDragNormal,
...
function updateBoneDrag(event: PointerEvent): boolean {
  ...
  if (raycaster.ray.intersectPlane(boneDragPlane, boneDragIntersection)) {
    ...
    position.x = boneDragIntersection.x;
    position.y = boneDragIntersection.y;
    position.z = boneDragIntersection.z;
    pointerDragged = true;
    updateParticleFields();
    markAnimationDirty();
    renderAnimationPose(frame.positions);
...
async function saveModelAs(): Promise<boolean> {
  ...
  try {
    ...
    if (desktopBridge.isAvailable()) {
      ...
      modelPath.textContent = result.path;
      model.dirty = false;
      updateStats();
      toast(`已另存为：${result.name}`, 'success');
      setStatus(`已保存：${result.path}`, 'success');
    } else {
...
async function overwriteModel(): Promise<boolean> {
  ...
  try {
    ...
    model.dirty = false;
    updateStats();
    toast(`已覆盖保存：${result.name}`, 'success');
    setStatus(`已覆盖：${result.path}`, 'success');
    return true;
  } catch (error) {
...
function switchAnimationPage(page: 'preview' | 'edit'): void {
  ...
  document
    ...
    .forEach((button) => button.classList.toggle('active', button.dataset.animationPage === page));
  document
    .querySelectorAll<HTMLElement>('[data-animation-panel]')
    .forEach((panel) => panel.classList.toggle('active', panel.dataset.animationPanel === page));
  updateBoneEditingState();
...
function createAnimation(): void {
  if (!model?.skeleton.length) {
    toast('请先载入一个带骨骼的模型。', 'warning');
    return;
  }
  const animation: RwrAnimation = {
    name: uniqueAnimationName('新动画'),
    loop: true,
    end: 1,
    speed: 1,
    speedSpread: 0,
  ...
  animationPlaying = false;
  animationPlayBtn.textContent = '▶';
  animationPlayBtn.disabled = false;
...
function duplicateAnimation(): void {
  ...
  activeAnimation = copy;
  selectedFrameIndex = 0;
  animationElapsed = copy.frames[0]?.time ?? 0;
  markAnimationDirty();
  updateAnimationSelect();
  updateAnimationUi();
  updateAnimationEditor();
  renderAnimationPose(copy.frames[0]?.positions);
  toast('动画已复制。', 'success');
}
...
function addKeyframe(duplicateCurrent = false): void {
  ...
  markAnimationDirty();
  updateAnimationSelect();
  selectKeyframe(selectedFrameIndex);
...
function updateParticlePosition(): void {
  ...
  const x = Number(particleXInput.value);
  const y = Number(particleYInput.value);
  const z = Number(particleZInput.value);
...
async function saveAnimationFile(): Promise<void> {
  ...
  try {
    ...
    if (desktopBridge.isAvailable()) {
      const result = await desktopBridge.saveTextFile(defaultName, xml);
      if (!result) return;
      currentAnimationFileName = result.name;
      animationDirty = false;
      updateAnimationWorkspaceStatus();
      toast(`动画已导出：${result.name}`, 'success');
      setStatus(`动画已保存：${result.path}`, 'success');
    } else {
      const blob = new Blob([xml], { type: 'application/xml;charset=utf-8' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      ...
      animationDirty = false;
      updateAnimationWorkspaceStatus();
      toast(`动画已导出：${defaultName}`, 'success');
    }
  } catch (error) {
...
function bindSettings(): void {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-settings-page]').forEach((button) =>
    button.addEventListener('click', () => {
      ...
      document
        ...
        .forEach((page) =>
          page.classList.toggle('active', page.dataset.page === button.dataset.settingsPage),
        );
    }),
  );
  ...
  element<HTMLInputElement>('#accentSetting').addEventListener('input', (event) => {
  ...
  document.querySelectorAll<HTMLInputElement>('[data-shortcut-input]').forEach((input) =>
    input.addEventListener('keydown', (event) => {
      ...
      const duplicate = shortcutActions.find(
        (candidate) => candidate !== action && settings.shortcuts[candidate] === shortcut,
      );
      if (duplicate) {
        toast(`快捷键 ${shortcut} 已被占用。`, 'warning');
        return;
      }
  ...
  element('#resetSettingsBtn').addEventListener('click', () => {
    settings = { ...defaultSettings, shortcuts: { ...defaultSettings.shortcuts } };
    populateSettingsForm();
    applySettings();
...
bindColorPickerSurface(colorSquare, (event) => {
  ...
  const sv = saturationValueFromPoint(
...
fileMenuBtn.addEventListener('click', (event) => {
  ...
  fileMenu.classList.toggle('hidden', !willOpen);
...
overwriteConfirmCheck.addEventListener('change', () => {
  confirmOverwriteBtn.disabled = !overwriteConfirmCheck.checked;
});
...
overwriteModal.addEventListener('pointerdown', (event) => {
  if (event.target === overwriteModal) closeOverwriteModal(false);
});
element('#openModelBtn').addEventListener('click', () => {
  closeFileMenu();
  void openFile('model');
});
...
rebindBtn.addEventListener('click', () => {
  if (!model) return;
  commit('骨骼绑定已重新计算', () => {
    const count = model!.rebindSkeleton();
    toast(`已为 ${count} 个体素重新绑定骨骼`, 'success');
  });
});
...
document
  ...
  .forEach((button) =>
    button.addEventListener('click', () =>
      switchAnimationPage(button.dataset.animationPage === 'edit' ? 'edit' : 'preview'),
    ),
  );
...
animationNameInput.addEventListener('change', () => {
  ...
  animationNameInput.value = activeAnimation.name;
  updateAnimationSelect();
});
animationEndInput.addEventListener('change', () => {
  ...
  animationEndInput.value = String(activeAnimation.end);
  markAnimationDirty();
  updateAnimationUi();
});
animationSpeedInput.addEventListener('change', () => {
  ...
  animationSpeedInput.value = String(activeAnimation.speed);
  markAnimationDirty();
});
animationSpreadInput.addEventListener('change', () => {
  if (!activeAnimation) return;
  activeAnimation.speedSpread = Math.max(0, Number(animationSpreadInput.value) || 0);
  animationSpreadInput.value = String(activeAnimation.speedSpread);
  markAnimationDirty();
});
animationLoopInput.addEventListener('change', () => {
  if (!activeAnimation) return;
  activeAnimation.loop = animationLoopInput.checked;
  markAnimationDirty();
});
...
keyframeTimeInput.addEventListener('change', () => {
  ...
  while (
    activeAnimation.frames.some(
      (candidate) => candidate !== frame && Math.abs(candidate.time - frameTime) < 0.000001,
    )
  )
    frameTime += 0.001;
...
particleSelect.addEventListener('change', () => {
  ...
  updateParticleFields();
  renderAnimationPose(currentKeyframe()?.positions);
});
[particleXInput, particleYInput, particleZInput].forEach((input) =>
  input.addEventListener('input', updateParticlePosition),
);
resetParticleBtn.addEventListener('click', () => {
  const frame = currentKeyframe();
  const particle = model?.skeleton[selectedParticleIndex];
  if (!frame || !particle) return;
  ensureFramePositions(frame);
  frame.positions[selectedParticleIndex] = { x: particle.x, y: particle.y, z: particle.z };
  updateParticleFields();
  markAnimationDirty();
  renderAnimationPose(frame.positions);
});
...
function rotateCameraView(deltaX: number, deltaY: number): void {
  ...
  cameraLookDirection.set(Math.sin(yaw) * Math.cos(pitch), Math.sin(pitch), -Math.cos(yaw) * Math.cos(pitch));
...
function moveCameraWithKeyboard(delta: number): void {
  const forwardAmount =
    Number(pressedCameraActions.has('cameraForward')) - Number(pressedCameraActions.has('cameraBack'));
  const rightAmount =
    Number(pressedCameraActions.has('cameraRight')) - Number(pressedCameraActions.has('cameraLeft'));
  ...
  const distance =
    Math.max(8, camera.position.distanceTo(controls.target) * 0.45) * settings.cameraSpeed * boost * delta;
  cameraMotion.multiplyScalar(distance);
  camera.position.add(cameraMotion);
  controls.target.add(cameraMotion);
}
